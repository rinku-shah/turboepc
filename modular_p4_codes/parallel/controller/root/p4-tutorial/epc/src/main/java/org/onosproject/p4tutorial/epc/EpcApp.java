...
//                                    ,defaultSwitch.getLong(),uePort,tmpArray2[0],Constants.ENODEB_SGW_PORT_MAP.get(defaultSwitch.getLong() + Constants.SEPARATOR + Constants.getSgwDpid(defaultSwitch)),Constants.getRanIp(defaultSwitch),sgw_teid,tmpArray[2]);
                        ...
                        // @vikas: in case of modular p4 code, since the state resides at DGW in the lookup tables(ue_context_rel_req_lookup_lb1_ub1 etc) we need to populate these tables on DGW
                        // since all the packets for attach are coming via DGW we can utilize its device ID itself to populate lookup tables 
                        DeviceId modular_offload_DGWswitchName = connectPoint.deviceId();
                        // lookup flag will check which tables to populate lb1_ub1 or lb2_ub2 as per the key 
                        int lookup_flag = Constants.getSgwswitchNameSeries_forlookup(dgw_dpId, Integer.parseInt(tmpArray[2]));
                        if(lookup_flag == 0){
                            log.debug("FATAL Error in lookup_flag in EpcApp");
                            System.exit(1);
                        }
                        ...
                        if (Integer.parseInt(tmpArray[2]) >= Constants.UE_LB && Integer.parseInt(tmpArray[2]) <= Constants.UE_UB) {
                                ...
                                fr.populate_lookup_if_tables(false,lookup_flag,appId,flowRuleService,modular_offload_DGWswitchName,Integer.parseInt(tmpArray[2]));
			            }
                    ...
                    case Constants.DETACH_REQUEST:
                        ...
                        if(status){
                            ...
                            // @vikas: in case of modular p4 code, since the state resides at DGW in the lookup tables(ue_context_rel_req_lookup_lb1_ub1 etc) we need to populate these tables on DGW
                            // since all the packets for Detach request are also coming via DGW we can utilize its device ID itself to populate lb_ub lookup tables 
                            DeviceId modular_offload_DGWswitchName1 = connectPoint.deviceId();

                            // lookup flag will check which tables to populate lb1_ub1 or lb2_ub2 as per the key 
                            int lookup_flag1 = Constants.getSgwswitchNameSeries_forlookup(dw, Integer.parseInt(tmpArray[2]));


                            // @offload design : remove populated auxilary tables from SGW switch in detach case
                            // while deleting rules we dont need match values so sending match parameters as 0 in all cases
                            if (Integer.parseInt(tmpArray[2]) >= Constants.UE_LB && Integer.parseInt(tmpArray[2]) <= Constants.UE_UB) {
                                    ...
                                    fr.populate_lookup_if_tables(true,lookup_flag1,appId,flowRuleService,modular_offload_DGWswitchName1,Integer.parseInt(tmpArray[2]));
			                }
                            build_response_pkt(connectPoint,srcMac,dstMac,ipv4Protocol,ipv4SourceAddress,udp_dstport,udp_srcport,response.toString());
