    ...
    apply {
        if (standard_metadata.ingress_port == CPU_PORT) {
          ...
          if (hdr.ipv4.isValid() && !hdr.gtpu.isValid()) {
                    ...
                    // @HO : sgw1 will only get service context release AFTER handover with changed epc traffic code
                    if(hdr.data.epc_traffic_code == 54){
...
control c_egress(inout headers hdr,
        ...
        apply {
                if (IS_I2E_CLONE(standard_metadata)) {
                            ...
                            // handle context release message
				//@Handover offload : we have to identify context release by changed epc traffic code which is 54 
                            if(hdr.data.epc_traffic_code == 54){
				...
				//@Handover offload : we have to identify service request by changed epc traffic code which is 54 
                            else if(hdr.data.epc_traffic_code == 57){
                                    // send the packet back to RAN
                                    ctxt_release_uekey_sgwteid_map.apply();
                            }

				//@Handover offload : we have to identify initial context setup response by changed epc traffic code which is 54 
                            else if(hdr.data.epc_traffic_code == 59){
                                    // send the packet back to RAN
