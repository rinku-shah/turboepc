    ...
    apply {
        if (standard_metadata.ingress_port == CPU_PORT) {
            ...
            if (hdr.ipv4.isValid() && !hdr.gtpu.isValid()) {
                ...
                // @vikas: since the code is made modular we have already done a table lookup at DGW so directly process the packet here

		        //   if((hdr.ue_context_rel_req.ue_num > 199) || (hdr.initial_ctxt_setup_resp.ue_key > 199)|| (hdr.ue_service_req.ue_key > 199)) { // HIT on the switch for uekey >99 i.e. all UE
                    
                    clone3(CloneType.I2E, I2E_CLONE_SESSION_ID, standard_metadata);

                    // handle context release message at local ONOS
...
control c_egress(inout headers hdr,
        ...
        action populate_ctxt_release_uekey_sgwteid_map(bit<32> sgwteid){
                ...
                hdr.ipv4.dstAddr = hdr.ipv4.srcAddr;
                 ...
                 // we need to send reply from sgw1,sgw2,sgw3,sgw4 as per the chain
		        ...
		        else if(hdr.ethernet.srcAddr == ran3 && (tmp_ue_num >= LB31 && tmp_ue_num <= UB31)){
                    hdr.ethernet.srcAddr = sgw31;
                    hdr.ipv4.srcAddr = s31_sgw_ipaddr;
                }
		        else if(hdr.ethernet.srcAddr == ran3  && (tmp_ue_num >= LB32 && tmp_ue_num <= UB32)){
                    hdr.ethernet.srcAddr = sgw32;
                    hdr.ipv4.srcAddr = s32_sgw_ipaddr;
                }
                 else if(hdr.ethernet.srcAddr == ran3  && (tmp_ue_num >= LB33 && tmp_ue_num <= UB33)){
                    hdr.ethernet.srcAddr = sgw33;
                    hdr.ipv4.srcAddr = s33_sgw_ipaddr;
                }

		        else if(hdr.ethernet.srcAddr == ran4  && (tmp_ue_num >= LB41 && tmp_ue_num <= UB41)){
                    hdr.ethernet.srcAddr = sgw41;
                    hdr.ipv4.srcAddr = s41_sgw_ipaddr;
                }
		        else if(hdr.ethernet.srcAddr == ran4  && (tmp_ue_num >= LB42 && tmp_ue_num <= UB42)){
