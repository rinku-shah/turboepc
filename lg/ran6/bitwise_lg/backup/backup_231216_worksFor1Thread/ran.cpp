...
bool serviceRequestLoopFlag = false; //set this true only for service requests
...
while(curTime < endTime){
		...
		if(attach_with_mme(ue, user, checkIntegrity)){ 	// Authentication
			//cout<<"After auth"<<endl;
			if(setUpTunnel || serviceRequestLoopFlag){
			//cout<<"Inside setup tunnel"<<endl;
				// Setup tunnel
				if(setUpTunnel)
				...
				//	cout<<"JUST BEFORE SEND DATA"<<endl;
					if(sendData){
						// Send data
