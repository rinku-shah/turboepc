...
control c_ingress(inout headers hdr,
    ...
    // // this table will send tha packet to SGW controller, though it is not required in SGW now but we keep it for completeness
...
/* definig offload tables here which will be used on SGW for Context Release and Service Request */
    ...
    apply {
            ...
            // if it is a UDP data traffic then forward it 
        if(hdr.ipv4.protocol == PROTO_UDP || hdr.ipv4.protocol == PROTO_TCP ){
            ...
            // Process all tunneled packets at SGW
                ...
                if(standard_metadata.ingress_port==0 && hdr.ipv4.dstAddr == s1u_sgw_addr && (hdr.udp.dstPort == 8001 || hdr.tcp.dst_port == 8001)){
                        //ip_op_tun_s2_uplink.apply();
                        standard_metadata.egress_spec = CPU_PORT;
                        return;
                }
                else if(standard_metadata.ingress_port==0 && hdr.ipv4.dstAddr == s1u_pgw_addr && (hdr.udp.dstPort == 8001 || hdr.tcp.dst_port == 8001)){
                        //ip_op_tun_s2_uplink.apply();
                        standard_metadata.egress_spec = 1;
                        return;
                }
                // if the ingress port is "p1"(1) then it a DOWNLINK tunnel packet (Sink -> RAN)
                else if(standard_metadata.ingress_port==1 && hdr.ipv4.dstAddr == s1u_sgw_addr && (hdr.udp.srcPort == 8001 || hdr.tcp.src_port == 8001)){
                        //ip_op_tun_s2_downlink.apply();
                        standard_metadata.egress_spec = CPU_PORT;
                        return;
                }
                else if(standard_metadata.ingress_port==1 && hdr.ipv4.dstAddr == s1u_dgw_addr && (hdr.udp.srcPort == 8001 || hdr.tcp.src_port == 8001)){
                        //ip_op_tun_s2_uplink.apply();
        ...
        if (standard_metadata.ingress_port == CPU_PORT) {
            ...
            else if( (hdr.ipv4.protocol == PROTO_TCP || hdr.ipv4.protocol == PROTO_UDP) && hdr.ipv4.dstAddr == s1u_dgw_addr){
                            //ip_op_tun_s2_uplink.apply();
                            standard_metadata.egress_spec = 0;
                            return;
                    }
                    else if((hdr.ipv4.protocol == PROTO_TCP || hdr.ipv4.protocol == PROTO_UDP) && hdr.ipv4.dstAddr == s1u_pgw_addr){
                            //ip_op_tun_s2_uplink.apply();
                            standard_metadata.egress_spec = 1;
                            return;
                    }
