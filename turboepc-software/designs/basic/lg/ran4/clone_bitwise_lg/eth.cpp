...
///ETHERNET
	struct ether_header *eh = (struct ether_header *) client_buffer; 
   ...
   const int *val = &one;
    ...
    int psize = sizeof(struct pseudo_header) + sizeof(struct udphdr) + sizeof(struct ether_header) + strlen(data);
    pseudogram =(char*) malloc(psize);
     
    memcpy(pseudogram , (char*) &psh , sizeof (struct pseudo_header));
    memcpy(pseudogram + sizeof(struct pseudo_header) , udph , sizeof(struct udphdr) + sizeof(struct ether_header) + strlen(data));
...
/* // Get the index of the interface to send on 
	...
	if (ioctl(client_socket, SIOCGIFINDEX, &if_idx) < 0)
	    perror("SIOCGIFINDEX");
	// Get the MAC address of the interface to send on 
	memset(&if_mac, 0, sizeof(struct ifreq));
	strncpy(if_mac.ifr_name, ifName, IFNAMSIZ-1);
	if (ioctl(client_socket, SIOCGIFHWADDR, &if_mac) < 0)
