...
string SEND_UE_TEID_HO = "26";
string ATTACH_ACCEPT_HO = "27";
...
vector<string> UserEquipment::setupTunnel(Client &user, bool doEncryption){
  ...
  do{
    ...
    do{
		...
		if(clearToSend[0] == ATTACH_ACCEPT){ // Handover done
 ...
 * This function initiates the tunnel setup procedure.
 */
vector<string> UserEquipment::setupTunnel_ho(Client &user, bool doEncryption){
  string send, receive, initial;
  vector<string> tmpArray;
  vector<string> clearToSend;
        //vector<string> tmpArray1;
  ...
  // ORIGINAL code: uncomment if required 
    ...
    // user.write_data(SEND_APN);
  if(doEncryption) {
    sendEncryptedData(user, SEND_APN_HO + SEPARATOR,  longToString(apn) + SEPARATOR + longToString(ue_key),"SEND_APN_HO");
  }
  // Receive UE IP address and SGW TEID from MME
  user.read_data();
  } while(user.timeoutFlag);
  ...
  if(tmpArray[0] == SEND_IP_SGW_TEID_HO){
    if(DO_DEBUG){
      cout<<" IP Address of UE="<<tmpArray[1]<<" and SGW TEID="<<tmpArray[2]<<" ue TEID="<<ue_teid<<endl;
    }
    //tmpArray.push_back(to_string(ue_teid));
    ...
    // ORIGINAL code: uncomment if required 
      ...
      // user.write_data(SEND_UE_TEID);
    if(doEncryption) {
      sendEncryptedData(user, SEND_UE_TEID_HO + SEPARATOR, longToString(ue_teid) + SEPARATOR + longToString(ue_key), "SEND_UE_TEID_HO");
    }
    ...
    if(clearToSend[0] == ATTACH_ACCEPT_HO){ //Not true (swapped _ho proc)--- Actually detach from previous chain done, and attach to current chain done --- Handover done
      tmpArray.push_back(clearToSend[1]); // clearToSend[1] => GUTI
      ...
      if(DO_DEBUG){
        cout<<"Handover completed"<<endl;
      }
    }else{
      cout<<"ERROR: Handover"<<endl;
      exit(1);
    }
    time_t seconds;
    seconds = time (NULL);
 ...
 */
void UserEquipment::initiate_ue_context_release(Client &user, int ue_num, string ue_ip, string sgw_teid, string ue_teid, int sinkUDPServerPort, bool networkServiceRequest,bool handoverdone){
	string send, receive;
	vector<string> tmpArray;
 ...
 */
string UserEquipment::send_ue_service_request(Client& user, int ue_num, string ue_ip, bool handoverdone){
	vector<string> clearToSend;
	int ue_teid;
...
string UserEquipment::receive_paging_request(Client& user, int ue_num, string ue_ip){
	...
	if (receive.find(SEPARATOR) != std::string::npos) {
		...
		if(tmpArray[0] == PAGING_REQUEST){
			...
			// @HO : making it false for completeness as we are not using paging for network service request
			ue_teid = send_ue_service_request(user, ue_num, ue_ip,false);
		}
	}
	return ue_teid;
}
