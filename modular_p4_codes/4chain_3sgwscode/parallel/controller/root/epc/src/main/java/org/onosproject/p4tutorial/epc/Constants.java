...
*/
/*    final static int LB11 = 100; //Select SGW11 when UE KEY is between LB11 & UB11
    final static int UB11 = 119;

    final static int LB12 = 120; //Select SGW12 when UE KEY is between LB12 & UB12
        final static int UB12 = 139;

        final static int LB13 = 140; //Select SGW21 when UE KEY is between LB13 & UB13
    final static int UB13 = 159;
*/
...
//			final static String STARTING_UE_IP_1 = "10.0.0.1"; //"10.129.41.103";
			...
			// @vikas : in parallel setup we have 3 SGWs in 1 chain 
			...
			final static int PGW_SINK_PORT = 9;  // SINK s3 connected on eth9 of PGW in container setup
			...
			{{
				...
				put(PGW_ID + SEPARATOR + SGW_ID_13, 3); // for switch S3(SGW-12) connected to S13(PGW) via port 2 of PGW
				...
				put(PGW_ID + SEPARATOR + SGW_ID_23, 4); // for switch S6(SGW-22) connected to S13(PGW) via port 4 of PGW
				...
				put(PGW_ID + SEPARATOR + SGW_ID_33, 6); // for switch S9(SGW-32) connected to S13(PGW) via port 6 of PGW
				...
				put(PGW_ID + SEPARATOR + SGW_ID_43, 8); // for switch S12(SGW-42) connected to S13(PGW) via port 8 of PGW
...
//
			...
			{{
				...
				put(DEFAULT_SWITCH_ID_1 + SEPARATOR + SGW_ID_13, 4);// for switch S3(SGW-12) connected to S1(ENODEB) via port 3 of Default Switch
				...
				put(DEFAULT_SWITCH_ID_2 + SEPARATOR + SGW_ID_23, 4);// for switch S6(SGW-22) connected to S2(ENODEB) via port 3 of Default Switch
				// chain3
				...
				put(DEFAULT_SWITCH_ID_3 + SEPARATOR + SGW_ID_33, 4);// for switch S9(SGW-32) connected to S3(ENODEB) via port 3 of Default Switch
				// chain4
				...
				put(DEFAULT_SWITCH_ID_4 + SEPARATOR + SGW_ID_43, 4);// for switch S12(SGW-42) connected to S4(ENODEB) via port 3 of Default Switch
			}};
...
//            getSgwDpid() fn can get "1", "s1", "s2", "s10", "12"
			...
			// lookup flag will check which tables to populate lb1_ub1 or lb2_ub2 r lb3_ub3 as per the key 
			public static int getSgwswitchNameSeries_forlookup(String dgw, int key){
				...
				int lookup_flag=0;  // lookup_flag will tell whether to populate entries on SGW1_1 switch or SGW1_2 switch or SGW1_3 switch after matching ue_key range
				String swid1 = "";
				if(dgw.charAt(0)=='s'){
						// means we get "s1", "s11" like switch numbers
						//log.warn("got switch name ={}",dgw);
...
//				String swid1 = Character.toString(sgw.charAt(1));  // assuming that switches will be from s1---s9 so char at position1 will give switchid which is an int
			...
			public static int getChainIDFromSGW(String sgw){
				int swid = Integer.parseInt(sgw);
				// @vikas : @modular 4chain : in case of parallel code both SGW1_1 and SGW1_2 are part of chain 1 so returning 0 in case of SGW_ID_2 as well.
				if ( (swid==(Constants.SGW_ID_11)) ||  (swid==(Constants.SGW_ID_12)) ||  (swid==(Constants.SGW_ID_13)) )
					return 0;
				else if ( (swid==(Constants.SGW_ID_21)) ||  (swid==(Constants.SGW_ID_22)) ||  (swid==(Constants.SGW_ID_23)) )
					return 1;
