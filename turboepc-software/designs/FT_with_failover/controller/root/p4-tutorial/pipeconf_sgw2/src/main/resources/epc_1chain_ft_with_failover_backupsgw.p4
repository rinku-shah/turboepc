    ...
    apply {
        if (standard_metadata.ingress_port == CPU_PORT) {
          ...
          if (hdr.ipv4.isValid() && !hdr.gtpu.isValid()) {
                ...
                // @ft : when SR' reaches local ONOS at SGW1_2, it generates a reply packet SR'' with ttl = 40 and forwards it on port 2 or port 1 of SGW1_2
                // Note that the UDP src and dst port are swapped in SR'' at local ONOS while generating the response packet

                // incoming packet SR/SR' can be from DGW(after failover) or SGW1_1(before failover) 
