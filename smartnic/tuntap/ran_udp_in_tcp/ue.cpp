...
// @vikas : passing second argument of sockfd as dummy value now
  user.write_data2(len,0);
 ...
 */
string UserEquipment::send_ue_service_request(Client& user, int ue_num, string ue_ip,int tcpdatasockfd){
  vector<string> clearToSend;
  int ue_teid;
  ...
  user.write_data2(len,tcpdatasockfd);
...
//      int ue_teid;
  ...
  while(!done){
    ...
    //mtx.unlock();
      ...
      ue_teid = ue_num;
      if(DO_DEBUG){
        cout<<" GENERATED teid, but currently set to ue_key: ue_teid="<<ue_teid<<endl;
      }
	...
	delay = delay + ((seconds) * 1000000 + useconds);      
      ...
      if(code_name2 == stoi(ATTACH_ACCEPT)){
...
string UserEquipment::receive_paging_request(Client& user, int ue_num, string ue_ip){
  ...
  if (receive.find(SEPARATOR) != std::string::npos) {
    ...
    if(tmpArray[0] == PAGING_REQUEST){
      ...
      // @vikas : passing second argument of sockfd as dummy value now
      ue_teid = send_ue_service_request(user, ue_num, ue_ip,0);
    }
  }
  return ue_teid;
}
