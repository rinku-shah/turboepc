...
control c_ingress(inout headers hdr,
   ...
   table ip_op_tun_s1_uplink{
    ...
    // @FT_with_failover : this table will be populated by root ONOS controller to toggle between primary_SGW and backup_SGW switch 
    action select_primary_backup_sgw_port(bit<9> egress_port_dgw){
        standard_metadata.egress_spec = egress_port_dgw;
        hdr.ipv4.ttl = hdr.ipv4.ttl - 1;
    }

    table toggle_primary_backup_sgw{
        key = {
             meta.metakey : exact;
        }
          actions = {
            select_primary_backup_sgw_port;
            NoAction;
        }
        size = 2048;
        default_action = NoAction();
    ...
    apply {
        if (standard_metadata.ingress_port == CPU_PORT) {
            ...
            // if the packet misses in the t_l3_fwd table then it means it either a Data packet or it is a context release/ Service request packet 
