...
int wait_latency = 528000;//2_98,0
//5_95,1
//6_94,2
//10_90,3
//25_75,4........15_85,4
...
int mix_num=0;	//choose the traffix mix from above traffic_options -> {0,1,2}  // 11 means only attach request will be sent
float a_prob = 0;
float s_prob = 0;
...
vector<unsigned long long> ho_registration_response_time;
...
int attNo = 0;
int detNo = 0;
int sreqNo = 0;//Counts number of requests served
int hoNo = 0;
...
void* multithreading_func(void *arg){
	...
	bool handover_t = false; // set it true for handover
	...
	Client user(threadId+100), sink_user(threadId+100);
	...
	while(curTime < endTime){
		...
		switch(traffic_type){
			...
			/* att-handover-det-loop1, */ 
			case 6: setUpTunnel_t = true;
					doDetach_t = true;
					sendData_t = true;
					assignIpToInterface_t = true;
					s1_release_t = true;
...
vector<string> setup_tunnel_ho(UserEquipment &ue, Client &user, bool doEncryption){
	return ue.setupTunnel_ho(user, doEncryption);
}
...
// @HO : handoverdone is a boolean variable which will tell if the handover to chain 1 is done,
// if done then we send different code to identify the ue_context_release message for chain1 (54)
// if handover not done then we send different epc traffic code for ue_context_release message (original code) (14)
void ue_context_release(UserEquipment &ue, Client &user, int ue_num, string ue_ip, string sgw_teid, string ue_teid, int currentPort, bool networkServiceRequest, bool handoverdone){
	ue.initiate_ue_context_release(user, ue_num, ue_ip, sgw_teid, ue_teid, currentPort, networkServiceRequest,handoverdone);
}

// @HO : handoverdone is a boolean variable which will tell if the handover to chain 1 is done,
// if done then we send different code to identify the ue_context_release message for chain1 (57)
// if handover not done then we send different epc traffic code for ue_context_release message (original code) (17)
string ue_service_request(UserEquipment &ue, Client &user, int ue_num, string ue_ip,  bool handoverdone){
	return ue.send_ue_service_request(user, ue_num, ue_ip,handoverdone);
}
...
int main(int argc, char *args[]){
	...
	ho_registration_response_time.resize(maxThreads, 0);
	...
	unsigned long long total_ho_reistration_time = 0;
    double average_sr_registration_time = 0.0;
    double average_ho_registration_time = 0.0;
	...
	for(int i=0;i<maxThreads;i++){
		...
		total_ho_reistration_time += ho_registration_response_time[i];
	...
	average_ho_registration_time = (total_ho_reistration_time*1.0)/(hoNo*1.0);
	...
	if(!fileExists(STATISTIC_FILE)){
		...
		data.append("EPOCH_TPT").append(COMMA).append("EPOCH_DELAY_ms").append(COMMA).append("ServiceRequestLatency").append(COMMA);
		data.append("#HO").append(COMMA);
		data.append("HO-Latency");
		data.append("\n");
	}
