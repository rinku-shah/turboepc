...
//	protected static Logger log = LoggerFactory.getLogger(SGW.class);
	...
	public synchronized String contactPGW(ApplicationId appId,FlowRuleService flowRuleService,DeviceId switchId,String sgw_dpId, String pgw_dpId, String apn){
		...
		if(Constants.DEBUG){
			...
			log.warn("SGW TEID of UE {} is {}",apn, sgw_teid);
	...
	public boolean detachUEFromSGW(ApplicationId appId,FlowRuleService flowRuleService,DeviceId SGWswitchId,String sgw_dpId, String pgw_dpId, int sgw_teid, String ue_ip) {
		...
		if(Constants.MYDEBUG){
			...
			log.warn("before get in SGW sgw_teid = {}",String.valueOf(sgw_teid)); //String.valueOf(sgw_teid)
		}
		    String pgw_teid1 = FT.get(dgwId, "SGW_PGW_TEID_MAP", String.valueOf(sgw_teid));
		    if(pgw_teid1==null){
		    	log.warn("recevied null value in line 229 in SGW");
			}
			else{
			...
			FT.del(Integer.parseInt(Constants.DETACH_REQUEST),dgwId, "SGW_PGW_TEID_MAP", String.valueOf(String.valueOf(sgw_teid)));   // TODO
	...
	public boolean detachUEFromSGW_ho(ApplicationId appId,FlowRuleService flowRuleService,DeviceId SGWswitchId,String sgw_dpId, String pgw_dpId, int sgw_teid, String ue_ip) {
			...
			//@HO: deleting only the rules from chain2 
			fr.insertUplinkTunnelForwardRule(true,appId,flowRuleService,SGWswitchId,sgw_teid,0,0,false);
			...
			// @HO : adding sgw_teid to resuable pool since UE has moved to chain1 after handover
			int chainId = Constants.getChainIDFromSGW(sgw_dpId);
			reusable_teids[chainId].add(sgw_teid);
