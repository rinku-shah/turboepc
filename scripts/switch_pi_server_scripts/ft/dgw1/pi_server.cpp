	...
	std::string controller_code = string_to_hex(packet->metadata(1).value());
     ...
     if(controller_code=="64"){
        ...
        if(master->election_id() == eidlocal) {
          ...
          stream->Write(response);
          response.release_packet();
          SIMPLELOG<<"PACKET Sent to **LOCAL** controller with election ID 64"<<std::endl;
          break;
         }
    }
...
/*
      ...
      if(trafficid == "0E" or trafficid == "11" or trafficid == "13") {
        ...
        SIMPLELOG<<"eidlocal = "<<eidlocal<<std::endl;
        if(master->election_id() == eidlocal) {
          SIMPLELOG<<"sending packet to local SGW1 controller"<<std::endl;
          auto stream = master->stream();
          p4::StreamMessageResponse response;
          response.set_allocated_packet(packet);
          stream->Write(response);
          response.release_packet();
          pkt_in_count++;
          SIMPLELOG<<"PACKET Sent to **LOCAL SGW1** controller"<<std::endl;
          // break;        
        }
        ...
        if(master->election_id() == eidroot) // root  
	{
	  ...
	  pkt_in_count++; 
		...
		SIMPLELOG<<"received invalid traffic_id MATCH packet\n"; 
	}
*/
     pkt_in_count++;
    }
