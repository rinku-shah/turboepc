    ...
    apply {
        ...
        if(hdr.ipv4.protocol == PROTO_UDP){
            ...
            // Process all tunneled packets at SGW
                ...
                if(standard_metadata.ingress_port==0 && hdr.ipv4.dstAddr == s1u_sgw_addr && hdr.udp.dstPort == 8001){
                        //ip_op_tun_s2_uplink.apply();
                        standard_metadata.egress_spec = CPU_PORT;
                        return;
                }
                else if(standard_metadata.ingress_port==0 && hdr.ipv4.dstAddr == s1u_pgw_addr && hdr.udp.dstPort == 8001){
                        //ip_op_tun_s2_uplink.apply();
                        standard_metadata.egress_spec = 1;
                        return;
                }
                // if the ingress port is "p1"(1) then it a DOWNLINK tunnel packet (Sink -> RAN)
                else if(standard_metadata.ingress_port==1 && hdr.ipv4.dstAddr == s1u_sgw_addr && hdr.udp.srcPort == 8001){
                        //ip_op_tun_s2_downlink.apply();
                        standard_metadata.egress_spec = CPU_PORT;
                        return;
                }
                else if(standard_metadata.ingress_port==1 && hdr.ipv4.dstAddr == s1u_dgw_addr && hdr.udp.srcPort == 8001){
                        //ip_op_tun_s2_uplink.apply();
