...
//// @rinku: read-data for att-det starts

void Client::read_data3(){
        int status=0;
        ...
        //eT = cT + (int) timeout;
        beginOff = clock();
        endOff = beginOff + timeout; //timeout in usec
        while(status!=17 || !tflag){    
            bzero(my_client_byte_buffer_3, BUFFER_SIZE);
            data_size = recvfrom(sock_raw, my_client_byte_buffer_3 , BUFFER_SIZE-1 , 0 , &saddr , (socklen_t*)&saddr_size);
            //Now process the packet        
            status=ProcessPacket3((unsigned char*)my_client_byte_buffer_3 , data_size);
            /*time(&cT);
            if (cT > eT) {
...
int Client::ProcessPacket3(unsigned char* buffer, int size)
{   
...
void Client::print_ip_header3(unsigned char* Buffer, int Size)
{
...
void Client::print_udp_packet3(unsigned char *Buffer , int Size)
{
    unsigned short iphdrlen;
    struct iphdr *iph = (struct iphdr *)(Buffer +  sizeof(struct ethhdr));
    iphdrlen = iph->ihl*4;
...
// TODO : only 8 bytes are copied in my_client_byte_buffer in print_udp_packet_2 why?? 
    ...
    memcpy(my_client_byte_buffer_3,(const char*) Buffer + header_size,11);

    // strcpy(my_client_byte_buffer, (const char*) Buffer + header_size);
}

///@rinku read-data for att-det ends
