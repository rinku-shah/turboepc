...
/* definig offload tables here which will be used on SGW for Context Release and Service Request */
    ...
    action populate_service_req_uekey_sgwteid_map(bit<32> sgwteid){
        ...
        // Updateing ipv4 totallen since we have added 10 Bytes more (4B sep + 6B sgwteid)
    ...
    apply {
        ...
        if (standard_metadata.ingress_port == CPU_PORT) {
            ...
            if(hdr.ethernet.etherType == 0x0806 && hdr.ethernet.dstAddr == 0x001111111111){
                    ...
                    return;
                     ...
                     //Outgoing ARP reply for p4client
        ...
        if(hdr.ethernet.etherType==TYPE_ARP){
          ...
          // if it is a TCP data traffic then forward it 
        if((hdr.ipv4.protocol == PROTO_TCP) && ((hdr.tcp.ctrl == 2) || (hdr.tcp.ctrl == 16) ) ){
        ...
        // if (hdr.ipv4.isValid()) {
                    ...
                    // @vikas : UDP in TCP : if it is a TCP packet and goin to TCP server at port 8080 then process it 
                    if(hdr.ipv4.protocol == PROTO_TCP && hdr.tcp.dst_port==TCP_Control_DST_PORT && (hdr.control_udp.dstPort == 5001)){
...
control c_egress(inout headers hdr,
        ...
        action populate_ctxt_release_uekey_sgwteid_map(bit<32> sgwteid){
                ...
                // @vikas : UDP in TCP : setting the outer Ipv4 and TCP headers invalid
                hdr.ipv4.setInvalid();
                hdr.tcp.setInvalid();

                hdr.ue_service_req.setInvalid();

                hdr.control_ipv4.ttl = 64;
                hdr.ethernet.dstAddr =  hdr.ethernet.srcAddr;
                hdr.control_ipv4.dstAddr = hdr.control_ipv4.srcAddr;

                 // we need to send reply from sgw1,sgw2,sge3,sgw4 as per the chain
                if(hdr.ethernet.srcAddr == ran1){
                    hdr.ethernet.srcAddr = sgw1;
                    hdr.control_ipv4.srcAddr = s1u_sgw_addr;
                }

                hdr.tmpvar.tmpUdpPort = hdr.control_udp.srcPort;
                hdr.control_udp.srcPort = hdr.control_udp.dstPort;
                hdr.control_udp.dstPort = hdr.tmpvar.tmpUdpPort;
                hdr.tmpvar.setInvalid();

                hdr.control_udp.length_ = 11 + UDP_HDR_SIZE;
                hdr.control_ipv4.totalLen =  hdr.control_udp.length_ + IPV4_HDR_SIZE;
        ...
        apply {
            ...
            if (standard_metadata.instance_type == (bit<4>) 0x8){
                            ...
                            if(hdr.data.epc_traffic_code == 14){
                                    ...
                                    hdr.control_udp.length_ = 7 + UDP_HDR_SIZE;
                                    hdr.control_ipv4.totalLen =  hdr.control_udp.length_ + IPV4_HDR_SIZE;
                                    // forwarding the cloned packet back to RAN on "p0"(0)
                                    standard_metadata.egress_port = 0;
