    ...
    def processPacket(self, p):
		...
		if data.epc_traffic_code == 14: #UE_Context_Release_Req
			try:
			    ...
			    with THRIFT_API_LOCK:
			        if data.param4 != 0:
			        	RTEInterface.Tables.DeleteRule(tbl_id, rule_name, default_rule, match, actions)
...
def main():
    ...
    RTEInterface.Connect(args.rpc_server,"10.129.2.201", args.rpc_port)
    ruleNum = 0
