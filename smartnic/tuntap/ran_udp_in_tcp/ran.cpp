...
bool doDetach = false;
bool sendData = false;			//keep either doDetach, or s1_release true for sending data
			...
			do{
				...
				do{
					if(s1_release_t){
						...
						//initiate_ue_context_release(Client &user, int ue_num, string ue_ip, string sgw_teid, string ue_teid, int sinkUDPServerPort, bool networkServiceRequest)
						int last_octet = ue_num;
						string first_part = "10.127.1.";
						string ue_ip = first_part + std::to_string(last_octet);
						int sgw_teid = (ue_num*10)+(ue_num%100);
						int ue_teid = ue_num;

						if(DO_DEBUG){
							cout<<"ue_key: "<< ue_num<< "ue_ip: "<< ue_ip <<" sgw_teid: "<<sgw_teid<<" ue_teid: "<<ue_teid<<endl;
						}
              ...
              string ueteid = ue_service_request(ue, user, ue_num, ue_ip,tcpdatasockfd); //returns newly generated ue_teid
						...
						//////////////////
						gettimeofday(&start2, NULL);
						 ue_context_release(ue, user, ue_num, ue_ip, to_string(sgw_teid), to_string(ue_teid), currentPort, networkServiceRequest);
					  ...
					  loop2--;
				}while(loop2>0); //end of do before s1 release
				time(&curTime);
				if(instrumentTptLat){
					inst_mtx.lock();
					time(&curTime);
...
string ue_service_request(UserEquipment &ue, Client &user, int ue_num, string ue_ip,int tcpdatasockfd){
	return ue.send_ue_service_request(user, ue_num, ue_ip,tcpdatasockfd);
}
...
int main(int argc, char *args[]){
	...
	// if ( (tcpdatasockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0 ) {
  ...
  //   if (connect(tcpdatasockfd, (struct sockaddr *)&servaddr, sizeof(servaddr)) != 0) { 
  //       printf("connection with the server failed...\n"); 
  //       exit(0); 
  //   }else {
  //       printf("connected to the server..\n"); 
	// 			cout<<"tcpdatasockfd = "<<tcpdatasockfd<<endl;
  //   }
	// /******************************************************************************************************************************/
