...
class DeviceState {
 public:
  ...
  void send_packet_in(p4::PacketIn *packet) {
   ...
   if(controller_code=="32"){
        Uint128 eidlocal(0,87);
        ...
        if(master->election_id() == eidlocal) {
          ...
          SIMPLELOG<<"PACKET Sent to **LOCAL** controller with election ID 74"<<std::endl;
          break;
         }
    }

      else if(controller_code=="64"){
                // if controller_code is 100 (64 in hex) then it means packet has to be sent to ROOT controller
                //Uint128 eidroot(0,21);   // uncomment and recompile while using 4 chain setup 
                Uint128 eidroot(0,12);  // for 1 chain only
...
/*
      ...
      if(trafficid == "0E" or trafficid == "11" or trafficid == "13") {
        // send to local controller
        Uint128 eidlocal(0,77);
        SIMPLELOG<<"eidlocal = "<<eidlocal<<std::endl;
        if(master->election_id() == eidlocal) {
          SIMPLELOG<<"sending packet to local controller"<<std::endl;
          auto stream = master->stream();
        ...
        } else if(trafficid == "0C" or trafficid == "01" or trafficid == "03" or trafficid == "14" or trafficid == "05" or trafficid == "07" or trafficid == "09" or trafficid == "18" or trafficid == "1B" or trafficid == "1D") {

        Uint128 eidroot(0,21);
